...
# Fetch all the dependencies without loading the code to have an independent layer
...
COPY --from=builder /code/target/release/mastodon-bluesky-sync /usr/bin/mastodon-bluesky-sync
...
ENTRYPOINT ["/usr/bin/mastodon-bluesky-sync"]
